...
app.get("/download/:code/:index", (req, res) => {
  ...
  // Generate a Cloudinary URL with the fl_attachment flag so the browser
  // downloads the file instead of opening it in a new tab.
  // We redirect the browser directly to Cloudinary — this avoids streaming
  // the entire file through our server (which times out on Render free tier)
  // while still forcing a Save-As dialog via the fl_attachment transformation.
  let downloadUrl;
  try {
    downloadUrl = cloudinary.url(file.publicId, {
      ...
      type          : "upload",
      secure        : true,
      flags         : "attachment:" + file.name.replace(/[^a-zA-Z0-9._-]/g, "_")
    });
  } catch (e) {
    // If SDK URL generation fails, append fl_attachment manually
    console.warn("Cloudinary URL generation failed:", e.message);
    downloadUrl = file.cloudUrl + (file.cloudUrl.includes("?") ? "&" : "?") + "fl_attachment";
  }

...
app.get("/download/:code/:index", (req, res) => {
  ...
  // IMPORTANT: Do NOT pass a filename to fl_attachment (i.e. no "attachment:foo.bar")
  // because Cloudinary treats the dot in the filename as a format extension in the
  // URL path, which causes a 400 Bad Request.
  // Instead use plain fl_attachment and let the browser use the publicId filename,
  // overriding it via the Content-Disposition header on THIS response before the redirect.
  let downloadUrl;
  try {
    downloadUrl = cloudinary.url(file.publicId, {
      ...
      type          : "upload",
      secure        : true,
      flags         : "attachment"          // no ":filename" — avoids the 400
    });
  } catch (e) {
    console.warn("Cloudinary URL generation failed, using raw URL:", e.message);
    // Append fl_attachment as a query param as last resort
  ...
  // Even though this is a redirect, Chrome/Firefox honour this header before following it.
  res.setHeader(
    "Content-Disposition",
